        ...
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`flex justify-center items-center w-12 h-12 ${
                    currentUser ? "bg-secondary text-white" : "bg-gray-100 text-gray-500"
            ...
            {showDropdown && currentUser && (
                <div
                    className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg z-20 py-1 border border-gray-100">
                    {/* User Info Header */}
                    <div className="px-4 py-3 border-b border-gray-100">
                        <p className="text-sm font-semibold text-textMain truncate">{currentUser.name}</p>
                        <p className="text-xs text-gray-500 truncate">{currentUser.email}</p>
                        <p className="text-xs text-gray-500 capitalize mt-1">Rol: {currentUser.role}</p>
                    </div>
                    ...
                    <nav className="py-1">
                        ...
                        >
                            <User size={16} className="mr-2 opacity-70"/> {/* Icon */}
                            Mi perfil
                        </button>
                        ...
                        {currentUser.role === "stylist" && (
                            <button
                                ...
                                className="flex items-center w-full text-left px-4 py-2 text-sm text-textMain hover:bg-gray-100"
                            >
                                <LayoutDashboard size={16} className="mr-2 opacity-70"/> {/* Icon */}
                                Panel de estilista
                            </button>
                        )}
                        ...
                        {currentUser.role === "client" && (
                            <button
                                ...
                                className="flex items-center w-full text-left px-4 py-2 text-sm text-textMain hover:bg-gray-100"
                            >
                                <Settings size={16} className="mr-2 opacity-70"/> {/* Icon */}
                                Preferencias
                            </button>
                        )}
                        ...
                        >
                            <LogOut size={16} className="mr-2 opacity-70"/> {/* Icon */}
                            Cerrar sesión
                        </button>
                    </nav>
                </div>
