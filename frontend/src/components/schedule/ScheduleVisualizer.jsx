...
function getGroupLabel(date) {
    ...
    if (isWithinInterval(parsed, { start: today, end: addDays(today, 7) }))
        return "Next 7 days"
    if (isWithinInterval(parsed, { start: today, end: addDays(today, 30) }))
        return "Next 30 days"
...
function ScheduleVisualizer({ tasks }) {
    ...
    const allLabels = Array.from(
        new Set([...Object.keys(grouped), ...hiddenGroups]),
    )

    const sortedLabels = allLabels.sort((a, b) => {
        const aDate = grouped[a]?.[0]
            ? parseISO(grouped[a][0].date)
            : new Date(0)
        const bDate = grouped[b]?.[0]
            ? parseISO(grouped[b][0].date)
            : new Date(0)
        return aDate - bDate
    })
    ...
    return (
        <div className="h-full overflow-hidden flex flex-col">
            <div className="bg-secondary/70 text-white px-6 py-3 rounded-t-lg rounded-b-lg sticky top-0 z-10 w-full">
                <h4 className="text-h4 font-heading">Citas agendadas</h4>
            </div>

            <div className="flex-1 overflow-x-auto overflow-y-hidden">
                <div className="flex gap-6 min-w-fit h-full px-4 py-3">
                    {sortedLabels.map((label) => {
                        ...
                        return (
                            <div
                                key={label}
                                className="w-64 flex-shrink-0 bg-secondary/70 rounded-md shadow-md flex flex-col max-h-[32rem]"
                            >
                                <div className="flex justify-between items-center px-3 pt-2 pb-1 relative">
                                    <div>
                                        <h5 className="text-subtitle-m font-bold text-white">
                                            {label}
                                        </h5>
                                        {grouped[label]?.[0] &&
                                            (label === "Today" ||
                                ...
                                <div className="space-y-4 overflow-y-auto px-3 pb-3 max-h-[calc(6*6rem)]">
                                    {grouped[label]?.map((task, i) => (
                                        ...
                                        >
                                            <p className="font-bold text-body-m">
                                                {task.service}
                                            </p>
